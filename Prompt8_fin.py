...
def get_phylo(aligned_sequences):
    ...
    tree_newick_str = tree.format('newick')

    #print(tree_file)
    #print(str(tree))
    return tree_newick_str
...
def treefile_to_list(file_str:str) -> list:
    """Read a Newick str in, split by commas. Returns a list."""
...
def main():
    ...
    #tree_file = treefile_to_list(treepath)
    seqIds = get_seqIds(fasta_file)
    identpairs = get_identpair(seqIds)
    tree_file = treefile_to_list(get_phylo(alignment))
